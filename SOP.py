...
class CaptureGraphicValues:
    ...
    def is_movement_key_pressed(self):
        """Check if any movement keys (W, S, A, D) are pressed."""
        return (keyboard.is_pressed('w') or 
    ...
    def searcherino(self):
        if self.is_movement_key_pressed():
            print("Movement key pressed, skipping firing...")
            return
        ...
        while True:
            if keyboard.is_pressed("shift"):  # Check for Left Shift key
                ...
                if win32api.GetAsyncKeyState(self.ViewGraph_hotkey) & 0x8000:
                    if not self.is_movement_key_pressed():
